...
class MissingPreference(NotificationPreference):
    """Push Rules for the 'missing' notification preference level."""

    def get_push_rules(self):
        logger.warning(
            f"No notification preference found for user {self.user_id} in room {self.room_id}"
        )
        return []


class NotificationPreferenceLookup:
    """Determines notification preference from push rules."""

    def __init__(self, user_id: str, room_id: str, rules: list[dict]):
        self.user_id = user_id
        self.room_id = room_id
        self.rules = rules

    def find(self) -> NotificationPreference:
        rule_ids = {rule["rule_id"] for rule in self.rules}
        if not rule_ids:
            return MissingPreference(self.user_id, self.room_id)
        if any("mentions_or_all" in rid for rid in rule_ids):
            return AllAndMePreference(self.user_id, self.room_id)
        if any("only_mentions" in rid for rid in rule_ids):
            return JustMePreference(self.user_id, self.room_id)
        if any(f"connect.room.disable_all;{self.room_id}" in rid for rid in rule_ids):
            return NoNotificationsPreference(self.user_id, self.room_id)
        if any(f"suppress.edits;{self.room_id}" in rid for rid in rule_ids):
