...
class MessageDecryptor:
    ...
    def _decrypt_message(self, message: dict[str, any]) -> None:
        ...
        try:
            ...
            new_content = event_json["content"].get("m.new_content")
            if new_content:
                encrypted_new_body = new_content.get("body")
                if encrypted_new_body:
                    new_content["body"] = self.key.decrypt(
                        encrypted_new_body.encode("utf-8")
                    ).decode("utf-8")

                encrypted_new_formatted_body = new_content.get("formatted_body")
                if encrypted_new_formatted_body:
                    new_content["formatted_body"] = self.key.decrypt(
                        encrypted_new_formatted_body.encode("utf-8")
                    ).decode("utf-8")
    ...
    def decrypt_event(self, event: EventBase) -> EventBase:
        ...
        try:
            ...
            logger.warning(
                "Returning event %s to client with undecrypted Fernet "
                "ciphertext still in content.body/formatted_body",
                event.event_id,
            )
            return event
