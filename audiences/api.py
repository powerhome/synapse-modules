...
from ..helpers.request_context import get_request_id, init_request_context, log_event
    ...
    async def _replace_username(self, extra_users_json: JsonDict):
        ...
        for json in extra_users_json:
            ...
            if scim_id not in mxids_by_scim_ids:
                log_event(
                    logger,
                    "SCIM ID not found in database mapping; user may not exist on v3",
...
class AudiencesCreateRoomServlet(AudiencesServlet):
    ...
    async def on_GET(self, request: SynapseRequest) -> Tuple[int, JsonDict]:
        ...
        init_request_context(request, action="get_rooms")
        log_event(
            logger,
        ...
        log_event(logger, "Get rooms request completed")
        return HTTPStatus.OK, body

    async def on_POST(self, request: SynapseRequest) -> Tuple[int, JsonDict]:
        requester = await self.auth.get_user_by_req(request)
        user_id = requester.user.to_string()
        init_request_context(request, action="create_room")
        ...
        log_event(logger, "Create room request completed", room_id=room_id)
        return HTTPStatus.OK, body
...
class AudiencesUsersServlet(AudiencesServlet):
    ...
    async def on_GET(self, request: SynapseRequest, key: str) -> Tuple[int, JsonDict]:
        ...
        try:
            ...
            log_event(logger, "Criteria users request completed", room_id=room_mxid)
            return HTTPStatus.OK, body
        except HttpResponseException as e:
            raise e.to_synapse_error() from e
...
class AudiencesContextServlet(AudiencesServlet):
    ...
    async def on_GET(self, request: SynapseRequest, key: str) -> Tuple[int, JsonDict]:
        init_request_context(request, action="get_context")
        room_mxid = await self._find_room_from_context(key)
        requester = await self.auth.get_user_by_req(request)
        await self.power_levels.verify(requester, room_mxid)
        ...
        log_event(logger, "Get context request completed", room_id=room_mxid)
        return HTTPStatus.OK, body

    async def on_PUT(self, request: SynapseRequest, key: str) -> Tuple[int, JsonDict]:
        requester = await self.auth.get_user_by_req(request)
        init_request_context(request, action="update_context")
        room_mxid = await self._find_room_from_context(key)
        await self.power_levels.verify(requester, room_mxid)
        ...
        log_event(
            logger,
            "Update context request received",
            key=key,
            room_id=room_mxid,
            requester=sender,
            extra_user_count=len(content.get("extra_users", [])),
        )
        ...
        try:
            ...
            if sender in self.bots:
                log_event(
                    logger,
                    "Update context request completed",
                    room_id=room_mxid,
                    context_event_emitted=False,
                )
                return HTTPStatus.OK, body
            else:
                event_dict: JsonDict = {
...
class AudiencesScimServlet(AudiencesServlet):
    ...
    async def on_GET(self, request: SynapseRequest) -> Tuple[int, JsonDict]:
        requester = await self.auth.get_user_by_req(request)
        init_request_context(request, action="scim_passthrough")
        log_event(
            logger,
            "SCIM request received",
            requester=requester.user.to_string(),
        )
        ...
        log_event(logger, "SCIM request completed")
        return HTTPStatus.OK, body
...
class AudiencesUserServlet(AudiencesServlet):
    ...
    async def on_GET(self, request: SynapseRequest) -> Tuple[int, JsonDict]:
        requester = await self.auth.get_user_by_req(request)
        init_request_context(request, action="scim_user_lookup")
        log_event(
            logger,
            "SCIM user lookup request received",
            requester=requester.user.to_string(),
        )
    ...
    async def _use_passthrough(self, request: SynapseRequest) -> Tuple[int, JsonDict]:
        log_event(
            logger,
            "Using passthrough for SCIM Users request; filter does not match exactly one user",
        )
        try:
            body = await self.hs.get_simple_http_client().get_json(
                uri=self._uri(request), headers=self.headers
...
class AudiencesBatchMembershipProcessorServlet(AudiencesServlet):
    ...
    async def on_PUT(self, request: SynapseRequest) -> Tuple[int, JsonDict]:
        ...
        """
        requester = await self.auth.get_user_by_req(request)
        init_request_context(request, action="batch_membership_changed")
        ...
        # Capture the request ID so it can be re-hydrated inside the background
        # process, where ContextVars set here are not guaranteed to propagate.
        request_id = get_request_id()
        log_event(
            logger,
            "Batch membership request received",
            room_id=room_id,
            requester=requester.user.to_string(),
        )
        ...
        if await self.archive_room_store.is_archived(room_id):
            log_event(
                logger,
                "Skipping batch membership processing for archived room",
                room_id=room_id,
            )
            return HTTPStatus.OK, {}

        # The evaluation runs synchronously within the request. The CPG holds a
        # per-room Redis lock for the duration of this PUT, so completing the
        # evaluation before responding makes that lock span the real critical
        # section. Cross-worker serialization does NOT rely on that lock alone:
        # the processor takes a Postgres-backed per-room claim (see
        # EvaluationClaims), so even when the CPG lock's guarantee breaks — a
        # PUT that outlives the client timeout, a retry landing on another
        # replica, an unarchive restore — a second evaluation of the same room
        # coalesces instead of racing. A failure propagates as a non-2xx
        # response so the CPG job can retry, which also re-covers any coalesced
        # request whose rerun was lost to the failure.
        handler = AudiencesBatchMembershipProcessor(self.config, self.module_api)
        await handler.process_batch_memberships(room_id, request_id)

        log_event(
            logger,
            "Batch membership processing completed",
            room_id=room_id,
        )
        return HTTPStatus.OK, {}
