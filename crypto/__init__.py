...
from synapse.rest.client.sync import SlidingSyncRestServlet, SyncRestServlet
...
# region GET /client/v3/rooms/{roomId}/event/{eventId}


class ConnectRoomEventServlet(RoomEventServlet):
    """Servlet that decrypts encrypted room events before returning them."""

    async def on_GET(
        self, request: SynapseRequest, room_id: str, event_id: str
    ) -> Tuple[int, JsonDict]:
        (status, response) = await super().on_GET(request, room_id, event_id)
        if status != 200:
            return (status, response)

        cc.decrypt_event(response)
...
# region GET /client/v3/rooms/{roomId}/context/{eventId}


class ConnectRoomEventContextServlet(RoomEventContextServlet):
    """Servlet that decrypts event context before returning it."""
    ...
    async def on_GET(
        ...
        cc.decrypt_event(response.get("event", {}))
        for event in response.get("events_before", []):
            cc.decrypt_event(event)
        for event in response.get("events_after", []):
            cc.decrypt_event(event)
...
# region POST /client/unstable/org.matrix.simplified_msc3575/sync


class ConnectSlidingSyncRestServlet(SlidingSyncRestServlet):
    """
    A subclass that decrypts messages fetched by the following endpoint:
    ...
    async def on_POST(self, request: SynapseRequest) -> Tuple[int, JsonDict]:
        (status, response) = await super().on_POST(request)
        if status != 200:
            return (status, response)

        for room in response.get("rooms", {}).values():
