...
class CorporateEventResource(JsonResource):
    ...
    def register_servlets(resource: HttpServer, hs: "HomeServer", config: dict) -> None:
        CorporateEventServlet(hs, config).register(resource)


class CorporateEventServlet(RestServlet):
    PATTERNS = [re.compile("^/_nitro/corporate_events")]
    CATEGORY = "Corporate Events requests"
    ...
    async def on_GET(self, request: SynapseRequest) -> Tuple[int, JsonDict]:
        ...
        uri = f"{self.bridge_base_url}/_nitro/corporate_events"
