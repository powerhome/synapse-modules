...
PROFILES_QUERY = """
    SELECT
        ...
        pp.avatar_url,
        (SELECT photos ->> 'value'
         FROM jsonb_array_elements(COALESCE(cp.data -> 'photos', '[]'::jsonb)) photos
         WHERE photos ->> 'type' = 'photo'
         LIMIT 1) AS scim_avatar_url,
        (SELECT photos ->> 'value'
         FROM jsonb_array_elements(COALESCE(cp.data -> 'photos', '[]'::jsonb)) photos
         WHERE photos ->> 'type' = 'thumbnail'
         LIMIT 1) AS scim_avatar_thumbnail_url,
