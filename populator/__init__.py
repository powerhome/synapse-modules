...
from .bulk_provisioning import BulkUserProvisioningResource
...
class Module:
    ...
    def __init__(self, config: dict[str, Any], api: ModuleApi) -> None:
        ...
        api.register_web_resource(
            path="/_populator/users",
            resource=BulkUserProvisioningResource(hs, populator),
        )
        api.register_web_resource(
            path="/_populator/user", resource=ProvisioningResource(hs, populator)
        )
        api.register_account_validity_callbacks(
            is_user_expired=populator.is_user_expired
        )
        logger.info("Populator module initialized, routes registered")
